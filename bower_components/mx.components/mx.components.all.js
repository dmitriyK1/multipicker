...
(function (w){
	...
	var MxMultiPickerControllerBase = function($timeout, $q, $element, $scope, internationalization) {
		...
		function onSelectionChange(type) {
			...
			if (type === 'remove') {
				vm.searchInput[0].setAttribute('disabled', 'disabled');

				setTimeout(function() {
					vm.searchInput[0].removeAttribute('disabled');
				}, 100);
			}
