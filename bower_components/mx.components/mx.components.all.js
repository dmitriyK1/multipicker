			...
			if (vm.errorController.$invalid || activeErrorsCount > 0) {
				...
				for (var i = 0; i < keys.length; i++) {
					...
					for (var j = 0; j < keyErrors.length; j++) {
						...
						_clientErrors.push(getErrorMessage('<div flex layout="column" class="error-message_field-name">' + (item.label || 'Unnamed form control') + '</div><div flex layout="column" class="error-message_error-text">&nbsp;&#8212; ' + message + '</div>'));
	...
	angular.module('mx.components').directive('mxLiteral', ['$filter', 'mx.internationalization', function ($filter, internationalization) {
		...
		function getTemplateByType(attr) {
			...
			switch (attr.type) {
				...
				case 'datetime':
					template =
						'<time class="literal-value" title="{{value | amDateFormat: \'dddd, MMMM Do YYYY, hh:mm\'}}" ng-show="::(format === \'timeAgo\' && value !== null)" am-time-ago="formattedValue" ##ngclass## style="min-width: ' + valueMinWidth + ';"></time>' +
						'<time class="literal-value" title="{{value | amDateFormat: \'dddd, MMMM Do YYYY, hh:mm\'}}" ng-show="::(format !== \'timeAgo\' || value === null)" ##ngclass## style="min-width: ' + valueMinWidth + ';">{{::formattedValue}}</time>';
					break;

				case 'html':
					template = '<div class="literal-value ' + singleLineClass + '" ng-bind-html="::value" ##ngclass##></div>';
					break;

				case 'currency':
					template = '<span class="currency-value literal-value" ##ngclass## style="min-width: ' + valueMinWidth + ';">{{::value}}&nbsp;{{::currencyCode}}</span>';
					break;

				default:
					template = '<span ng-bind="formattedValue" class="literal-value ' + singleLineClass + '" ##ngclass## style="min-width: ' + valueMinWidth + ';"></span>';
					break;
			}
